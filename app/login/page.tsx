    ...
    <main style={{ minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", padding:"24px" }}>
      <div style={{ width:"100%", maxWidth:"380px" }}>
        ...
        <form onSubmit={e => e.preventDefault()} style={{ display:"flex", flexDirection:"column", gap:"12px" }}>
          ...
          {error && (
            <p style={{ fontSize:"12px", color: error.includes("Check") ? "var(--accent)" : "var(--error)" }}>
              {error}
            </p>
          )}
          <button type="button" onClick={handleLogin} disabled={loading} style={btnPrimary}>
            {loading ? "..." : "Sign in"}
          </button>
          <button type="button" onClick={handleSignup} disabled={loading}
            style={{ ...btnPrimary, background:"var(--surface)", color:"var(--text)", border:"1px solid var(--border)" }}>
            Create account
          </button>
        </form>
      </div>
